...
使用规则：
1. **引用核对**：任何模式引用诗句前，先查精读库（人工核定的通行版原文），没有再查底库。注意：底库为古籍版本，部分诗与通行版有字词差异（如《静夜思》底库是宋本"床前看月光……举头望山月"，通行版"床前明月光……举头望明月"），核对时以精读库为准，底库差异可作为知识点顺带说明。底库也没有的才用自身知识，并标注"待核实"。
